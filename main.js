...
//         // Store the new configuration as the baseline for future modifications
//         simulator.storeCurrentAsOriginal();
...
// Update the randomizeForces function in main.js
async function randomizeForces() {
    ...
    try {
        await simulator.randomizeForces(forceParams); // Pass the global parameters
...
// Replace your restartWithNewConfiguration function in main.js with this:
...
async function restartWithNewConfiguration() {
    ...
    try {
        ...
        // Generate the new configuration using your lava lamp function
        const newConfig = generateLavaLampConfiguration(numTypes, numParticles, forceScale, radius);
        ...
        // IMPORTANT: Create a completely new simulator instance
        // Don't try to reuse the old one, as it might have the wrong particle count/types
        ...
        if (initialized) {
            ...
            simulator.storeCurrentAsBaseline();
        ...
        try {
            const fallbackConfig = generateLavaLampConfiguration(4, 10000, 1.0, 20);
            console.log("Fallback config generated:", fallbackConfig);

            simulator = new ParticleLifeSimulator('webgpu-canvas', fallbackConfig);
            const initialized = await simulator.initialize();

            if (initialized) {
                simulator.storeCurrentAsBaseline();
                syncSlidersWithConfig();
                updateConfigDisplay(fallbackConfig);
                if (wasRunning) {
                    simulator.start();
                }
                updateButtonStates();
...
// Make sure you have this function available (it should be in your main.js already)
// If you don't have it, you need to add it:
...
// Update startSimulation function in main.js
async function startSimulation() {
    try {
        ...
        if (initialized) {
            ...
            // Automatically set the initial JSON config as baseline
            simulator.storeCurrentAsBaseline();
...
// Enhanced function to setup the force parameter sliders with visual feedback
function setupForceParameterSliders() {
    const defaults = {
        strengthModifier: 110,
        radiusRange: 25,
        collisionStrengthRange: 750,
    ...
    if (strengthSlider) {
        strengthSlider.addEventListener('input', () => {
            forceParams.strengthModifier = parseFloat(strengthSlider.value);
    ...
    if (radiusSlider) {
        radiusSlider.addEventListener('input', () => {
            forceParams.radiusRange = parseFloat(radiusSlider.value);
    ...
    if (collisionStrengthSlider) {
        collisionStrengthSlider.addEventListener('input', () => {
            forceParams.collisionStrengthRange = parseFloat(collisionStrengthSlider.value);
    ...
    if (collisionRadiusSlider) {
        collisionRadiusSlider.addEventListener('input', () => {
            forceParams.collisionRadiusRange = parseFloat(collisionRadiusSlider.value);
...
document.addEventListener('DOMContentLoaded', () => {
    configureCanvas();
    setupSliders();
    setupForceParameterSliders(); // Add this line
    ...
    document.getElementById('reset-btn').addEventListener('click', async () => {
        if (simulator) {
            ...
            try {
                await simulator.resetWithModifiedBaseline(forceParams);
                updateButtonStates();
    ...
    // Add this new button handler in your DOMContentLoaded event listener
    document.getElementById('reset-positions-btn').addEventListener('click', () => {
        if (simulator) {
            ...
            simulator.resetPositionsOnly();
            updateButtonStates();
        }
    });
    // Add this in your DOMContentLoaded event listener:
    const resetOriginalBtn = document.getElementById('reset-to-original-btn');
    if (resetOriginalBtn) {
        resetOriginalBtn.addEventListener('click', () => {
            if (simulator) {
                simulator.resetToOriginalForces();
    ...
    // Fix new complete config button
